...
export default function StatisticsChart({ series }: { series: Series[] }) {
  ...
  const options: ApexOptions = {
    ...
    yaxis: {
      ...
      title: {
        text: selectedSeriesIndex === 0 ? "Cubic Meters" : "Number of TMs",
        style: {
          fontSize: "12px",
          color: "#6B7280",
        },
      ...
      <div className="flex flex-col gap-5 mb-6 sm:flex-row sm:justify-between">
        <div className="w-full">
          ...
          <p className="mt-1 text-gray-500 text-theme-sm dark:text-gray-400">
            {selectedSeriesIndex === 0 ? "Pumping quantity per month" : "Transit mixers usage per month"}
          </p>
        </div>
        <div className="flex items-start w-full gap-3 sm:justify-end">
          <ChartTab onSelect={setSelectedSeriesIndex} />
        </div>
      </div>

      <div className="max-w-full overflow-x-auto custom-scrollbar">
        <div className="min-w-[1000px] xl:min-w-full">
          <ReactApexChart options={options} series={currentSeries} type="area" height={250} />
        </div>
      </div>
    </div>
  );
